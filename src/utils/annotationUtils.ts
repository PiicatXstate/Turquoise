...
export function getTextNodes(node: Node): Text[] {
  ...
  while (currentNode = walker.nextNode()) {
    console.log(`getTextNodes: found text node with length ${currentNode.nodeValue?.length}`, currentNode);
...
export function getTextNodesInRange(range: Range): Text[] {
  console.log(`getTextNodesInRange: range from ${range.startOffset} to ${range.endOffset} in container`, range.startContainer);
  const textNodes: Text[] = [];
  ...
  const treeWalker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode: function(node) {
        ...
        console.log(`getTextNodesInRange: node at position ${rangeStartCompare}, ${rangeEndCompare}`, node);
  ...
  while (currentNode = treeWalker.nextNode()) {
    console.log(`getTextNodesInRange: accepted node with text "${currentNode.nodeValue?.substring(0, 50)}..."`);
    textNodes.push(currentNode as Text);
  }
  
  console.log(`getTextNodesInRange: found ${textNodes.length} nodes in range`);
  return textNodes;
}
...
export function getTextOffsets(range: Range, rootNode: Node): { start: number, end: number } {
  console.log(`getTextOffsets: starting with range`, range, `and rootNode`, rootNode);
  ...
  for (let i = 0; i < textNodes.length; i++) {
    ...
    console.log(`getTextOffsets: node ${i} has length ${nodeLength}, total so far: ${totalLength}`);
    ...
    if (startOffset === -1 && node === range.startContainer) {
      ...
      if (range.startContainer.nodeType === Node.TEXT_NODE) {
        ...
        console.log(`getTextOffsets: found start offset at ${startOffset} (nested text node)`);
      }
    }
    ...
    if (endOffset === -1 && node === range.endContainer) {
      ...
      if (range.endContainer.nodeType === Node.TEXT_NODE) {
        endOffset = totalLength + (range.endContainer.nodeValue?.length || 0);
        console.log(`getTextOffsets: found end offset at ${endOffset} (nested text node)`);
        break;
      }
    }
  ...
  if (startOffset !== -1 && endOffset === -1) {
    for (let i = 0; i < textNodes.length; i++) {
      ...
      if (endOffset === -1 && node === range.endContainer) {
        endOffset = totalLength + range.endOffset;
        console.log(`getTextOffsets: found end offset at ${endOffset} (in second pass)`);
        break;
      }
      totalLength += nodeLength;
    }
  ...
  // 验证是否找到有效偏移
  if (startOffset === -1 || endOffset === -1) {
    console.error(`getTextOffsets: failed to find offsets. start: ${startOffset}, end: ${endOffset}`);
    ...
    if (backupOffsets) {
      console.log(`getTextOffsets: using backup method results`, backupOffsets);
      return backupOffsets;
    }
    
    throw new Error("Range boundaries are not in text nodes within rootNode");
  }

  console.log(`getTextOffsets: returning start: ${startOffset}, end: ${endOffset}`);
 ...
 */
function getTextOffsetsBackup(range: Range, rootNode: Node): { start: number, end: number } | null {
  ...
  try {
    ...
    console.log(`getTextOffsetsBackup: backup offsets start: ${startOffset}, end: ${endOffset}`);
...
export function createRangeFromOffsets(
  ...
  console.log(`createRangeFromOffsets: rootNode`, rootNode, `start: ${startOffset}, end: ${endOffset}`);
  ...
  for (let i = 0; i < textNodes.length; i++) {
    ...
    console.log(`createRangeFromOffsets: node ${i} length ${nodeLength}, currentOffset: ${currentOffset}`);
    
    if (startOffset >= currentOffset && startOffset < currentOffset + nodeLength) {
      startNode = node;
      startNodeOffset = startOffset - currentOffset;
      console.log(`createRangeFromOffsets: found start node at index ${i}, offset ${startNodeOffset}`);
      break;
    }
  ...
  for (let i = 0; i < textNodes.length; i++) {
    ...
    if (endOffset >= currentOffset && endOffset <= currentOffset + nodeLength) {
      ...
      console.log(`createRangeFromOffsets: found end node at index ${i}, offset ${endNodeOffset}`);
      break;
    }
    currentOffset += nodeLength;
  }
  ...
  if (!startNode || !endNode) {
    console.error(`createRangeFromOffsets: failed to find nodes. startNode: ${startNode}, endNode: ${endNode}`);
  ...
  try {
    ...
    console.log(`createRangeFromOffsets: created range from ${range.startOffset} to ${range.endOffset}`);
    return range;
  } catch (error) {
...
function createRangeFromOffsetsBackup(
  ...
  try {
    ...
    console.log(`createRangeFromOffsetsBackup: adjusted offsets start: ${adjustedStart}, end: ${adjustedEnd}`);
    ...
    if (startNode && endNode) {
      ...
      console.log(`createRangeFromOffsetsBackup: created range using backup method`);
      return range;
    }
  } catch (error) {
    console.error("createRangeFromOffsetsBackup: error using backup method", error);
  }
...
export function getContext(text: string, position: number, length: number, contextLength: number = 30): { 
  ...
  console.log(`getContext: position ${position}, length ${length}, contextLength ${contextLength}`);
  ...
  console.log(`getContext: before: "${result.before}", after: "${result.after}"`);
  return result;
}
...
export function findTextWithContext(
  ...
  console.log(`findTextWithContext: searching for "${searchText}" with context before: "${contextBefore}", after: "${contextAfter}"`);
  ...
  console.log(`findTextWithContext: clean context before: "${cleanContextBefore}"`);
  console.log(`findTextWithContext: clean context after: "${cleanContextAfter}"`);
  
  if (!cleanSearchText) {
    console.log(`findTextWithContext: clean search text is empty, returning -1`);
    return -1;
  }
  ...
  while (position < text.length) {
    ...
    matchCount++;
    console.log(`findTextWithContext: found match ${matchCount} at position ${position}`);
    ...
    const afterEnd = Math.min(text.length, position + searchText.length + cleanContextAfter.length);
    ...
    console.log(`findTextWithContext: actual context before: "${cleanBefore}"`);
    console.log(`findTextWithContext: actual context after: "${cleanAfter}"`);
    
    // 计算上下文匹配度
    const beforeMatchScore = calculateMatchScore(cleanBefore, cleanContextBefore);
    const afterMatchScore = calculateMatchScore(cleanAfter, cleanContextAfter);
    
    console.log(`findTextWithContext: before match score: ${beforeMatchScore}`);
    console.log(`findTextWithContext: after match score: ${afterMatchScore}`);
    ...
    // 如果找到完美匹配，立即返回
    if (matchScore > 0.9) {
      console.log(`findTextWithContext: perfect match found at position ${position}`);
      return position;
    }
    ...
    if (matchScore > bestMatchScore) {
      ...
      console.log(`findTextWithContext: new best match at position ${position} with score ${bestMatchScore}`);
    }
  ...
  if (bestMatchScore > 0.7) {
    console.log(`findTextWithContext: returning best match at position ${bestMatchPosition} with score ${bestMatchScore}`);
...
function calculateMatchScore(str1: string, str2: string): number {
  if (!str1 || !str2) {
    console.log(`calculateMatchScore: one or both strings are empty, returning 0`);
    return 0;
  }
  
  // 如果其中一个字符串是空的，匹配度为0
  if (str1.length === 0 || str2.length === 0) {
    console.log(`calculateMatchScore: one or both strings have zero length, returning 0`);
    return 0;
  }
  ...
  console.log(`calculateMatchScore: "${str1}" vs "${str2}" -> editDistance: ${editDistance}, maxLength: ${maxLength}, score: ${score}`);
...
function levenshteinDistance(a: string, b: string): number {
  console.log(`levenshteinDistance: calculating distance between "${a}" and "${b}"`);
...
export function saveAnnotation(annotation: Annotation): void {
  ...
  if (existingIndex !== -1) {
    ...
    console.log(`saveAnnotation: created new annotation with ID ${annotation.id}`);
  }
  
  localStorage.setItem(key, JSON.stringify(annotations));
  console.log(`saveAnnotation: saved ${annotations.length} annotations total`);
}
...
export function deleteAnnotation(bookId: string, annotationId: string): void {
  console.log(`deleteAnnotation: bookId ${bookId}, annotationId ${annotationId}`);
  ...
  localStorage.setItem(key, JSON.stringify(annotations));
  console.log(`deleteAnnotation: removed ${initialCount - annotations.length} annotation(s)`);
}
...
export function getAnnotationsForChapter(bookId: string, chapterHref: string): Annotation[] {
  console.log(`getAnnotationsForChapter: book ${bookId}, chapter ${chapterHref}`);
  const annotations = loadAnnotations(bookId);
  const chapterAnnotations = annotations.filter(ann => ann.chapterHref === chapterHref);
  console.log(`getAnnotationsForChapter: found ${chapterAnnotations.length} annotations for this chapter`);
...
export function cleanupStaleAnnotations(bookId: string, chapterHref: string, chapterText: string): void {
  console.log(`cleanupStaleAnnotations: checking for stale annotations in book ${bookId}, chapter ${chapterHref}`);
  const annotations = getAnnotationsForChapter(bookId, chapterHref);
  let removedCount = 0;
  
  annotations.forEach(annotation => {
    console.log(`cleanupStaleAnnotations: checking annotation ${annotation.id} with text "${annotation.text.substring(0, 50)}..."`);
    ...
    // 如果匹配度太低，删除这个注释
    if (position === -1) {
      console.log(`cleanupStaleAnnotations: removing stale annotation ${annotation.id}`);
      deleteAnnotation(bookId, annotation.id);
      removedCount++;
    } else {
      console.log(`cleanupStaleAnnotations: annotation ${annotation.id} is still valid at position ${position}`);
    }
  });
  
  console.log(`cleanupStaleAnnotations: removed ${removedCount} stale annotations`);
}